...
def invert_to_chars(sents, inv_ctc_map):
    """Convert encoded label tensor back to a list of gloss strings."""
    sents = sents.detach().numpy() if isinstance(sents, torch.Tensor) else sents
    outs = []
    for row in sents:
        for x in row:
            if x == 0:
...
def convert_text_for_ctc(dataset_name, train_csv, dev_csv):
    """
    Read annotation CSVs, build vocabulary, and encode glosses for CTC training.
    Supports isharah/csl format (id|gloss) and generic format (id|annotation).
    """
    ...
    def encode(df):
        ...
        df["enc"] = df[gloss_col].apply(lambda x: [vocab_map[g] for g in x.split() if g in vocab_map])
        return df[["id", "enc"]]
